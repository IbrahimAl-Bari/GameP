...
const Xbox = () => {
    ...
    const fetchGames = async (query = '') => {
        ...
        try {
            ...
            setSpecialgames(data.results.filter(game => game.parent_platforms?.some(p => p.platform.name.includes("Xbox"))));
    ...
    const mostpopularGames = async () => {
        try {
            ...
            setMostPopular(data2.results.filter(game => game.parent_platforms?.some(p => p.platform.name.includes("Xbox"))));
            ...
            <div className="wrapper">
                ...
                <section className="trending">
                    {debouncedSearchTerm ? null : <div>
                        <h2>Best Rating Xbox Games According To <span className="text-gradient">RAWG</span> :
                        </h2>
                ...
                <section className="all-games">
                    ...
                    {debouncedSearchTerm ? (<h2>Search Results for "<span className="text-gradient">{searchTerm}</span>"</h2>) : (
                        <h2>Best <span className={'text-gradient'}>Xbox</span> Games :</h2>)}
